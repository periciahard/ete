(function(){
 ...
 function bind(){
  ...
  A().$('#validateDataGate')&&(A().$('#validateDataGate').onclick=()=>{try{if(window.Importacao.pending) renderValidation(window.Importacao.pending); else renderValidation(rowsToAssessment(parseTextTable(A().$('#pasteData').value)));}catch(e){A().status('#importGate',A().safe(e.message),'error');}});
