(function(){
  ...
  const App={VERSION, STORE, state:load(), $, $$, safe, norm, letter, desc, pct, status, download, toTSV, uid,
    ...
    renderChangelog(){const box=$('#changelogBox'); if(box) box.innerHTML='<h3>Novidades da V46</h3><ul><li>Coordenação com plano de ação mais detalhado, turmas críticas, alunos prioritários e acompanhamento por descritores.</li><li>Aba TRI com explicação didática de discriminação e leitura de coerência dos estudantes.</li><li>Upload do PDF/TXT da prova para mapear questões, descritores e nível estimado dos itens.</li><li>Dificuldade final recalibrada pela taxa de acertos da turma quando houver dados importados.</li><li>Relatórios e sugestões pedagógicas mais orientados à tomada de decisão.</li></ul>';},
