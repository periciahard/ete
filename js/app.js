...
function formatDescriptorLine(d){let info=findDescriptor(d.d);return `- ${d.d}: ${d.p}% de acertos (${d.ac}/${d.total}) | Questões: ${d.qs.join(', ')}${info?` | ${info.texto}`:''}`}
...
function interventionBlock(){let crit=descriptorStats().filter(d=>d.d!='Sem descritor').slice(0,5);if(!crit.length)return 'Não há descritores mapeados suficientes para sugerir intervenção específica.';return crit.map((d,i)=>{let info=findDescriptor(d.d);return `${i+1}. ${d.d} — ${d.p}% de acertos\n   Habilidade: ${info?info.texto:'descritor não localizado na biblioteca'}\n   Ação sugerida: ${info?info.intervencao:'retomada conceitual, atividade guiada e verificação curta.'}\n   Alunos prioritários nesse descritor: ${studentsByDescriptor(d.d).slice(0,12).join(', ')||'não identificado'}`}).join('\n\n')}
...
function criticalDescriptorObjectsForStudent(s){
  ...
  return uniq.map(code=>findDescriptor(code) || {codigo:code,texto:'Descritor não localizado na biblioteca da disciplina selecionada.',explicacao:'Verifique se a disciplina escolhida corresponde à avaliação importada.',intervencao:'Conferir disciplina, descritor e gabarito; depois realizar exercícios guiados.',disciplina:currentDiscipline()});
}
